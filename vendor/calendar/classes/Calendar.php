<?php
namespace vendor\calendar\classes;
...
class Calendar extends Event_Subject {
	...
	/**
	 * Get the URL for a previous year link
	 *
	 * @return  string
	 */
	public function prev_year_url()
	{
		$date  = mktime(0, 0, 0, $this->month, 1, $this->year - 1);
		$month = date('n', $date);
		$year  = date('Y', $date);
		$url   = self::query(array('m' => $month, 'y' => $year));
	 ...
	 * @return  string
	 */
	public function month_url($month, $year = null)
	{
		$date  = mktime(0, 0, 0, $month, 1, $year? $year : $this->year);
		$month = date('n', $date);
		$year  = date('Y', $date);
		$url   = self::query(array('m' => $month, 'y' => $year));
	...
	{
		return Url::to(['/salon/salon/calendar/']);
	}
	...
	/**
	 * Get the URL for a next year link
	 *
	 * @return  string
	 */
	public function next_year_url()
	{
		$date  = mktime(0, 0, 0, $this->month, 1, $this->year + 1);
		$month = date('n', $date);
		$year  = date('Y', $date);
