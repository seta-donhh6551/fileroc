...
class BaseImage
{
     ...
     * @return ImageInterface
     */
    public static function thumbnail($filename, $firstWidth, $firstHeight, $auto = false, $mode = ManipulatorInterface::THUMBNAIL_OUTBOUND)
    {
        //Modify by haanhdon
        $imgSize = getimagesize($filename);
        $imgWidth = $imgSize[0];
        $imgHeight = $imgSize[1];
        $percen = $imgWidth/$imgHeight; 
       
        $width = $firstWidth;
        $height = $firstHeight;
        if($auto){
            $height = ceil($firstWidth/$percen);
            if($height > 375){
                $height = 375;
            }
        }
